...
def generate_partition(odd_type, part=None):
    if odd_type == "cylinder":
    ...
    elif odd_type == "cuboid":
        if part is None:
            part = [1, 1, 1]
        # dim 0: dir_coeff in [0, 1]
        # dim 1: x_norm   in [-1, 1]
        # dim 2: z_norm   in [-1, 1]
        input_min = np.array([0.0, -1.0, -1.0], dtype=np.float32)
        input_max = np.array([1.0,  1.0,  1.0], dtype=np.float32)
        part = np.array(part)
        ...
        for i in range(part[0]):
            for j in range(part[1]):
                for k in range(part[2]):
                    lb = np.array([edges[0][i], edges[1][j], edges[2][k]])
                    ub = np.array([edges[0][i+1], edges[1][j+1], edges[2][k+1]])
                    center = (lb + ub) * 0.5
                    lower_bounds.append(lb)
                    upper_bounds.append(ub)
                    centers.append(center)

        lower_bounds = np.stack(lower_bounds)
        upper_bounds = np.stack(upper_bounds)
